...
;(function(){
  ...
  function buildLocalPdf(quoteInput, localTemplate){
    ...
    const totalAvista   = Number(payload.total_avista   || 0) || subtotal
  ...
  function buildPedidoLocalPdf(quoteInput, localTemplate){
    ...
    modelos.forEach((m, idx) => {
      ...
      if(descP3){
        ...
        doc.splitTextToSize(descP3, W - margin * 2 - 60).forEach(line => {
      ...
      if(inclP3.length){
        ...
        doc.splitTextToSize('Incluso: ' + inclP3.join(' / '), W - margin * 2).forEach(line => {
          ensureSpace(5); doc.text(line, margin, y); y += 4.5
        })
        doc.setTextColor(60, 60, 60)
      }
    ...
    y += 2
    rowLR('Total à vista:',   fmt(subtotalGeral))
    rowLR('Total no cartão:', fmt(subtotalGeral * 1.10))
