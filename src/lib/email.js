...
function buildWelcomeHtml(name, empresa) {
  const body = `
    ...
    <p style="margin:0 0 24px;color:#64748b;font-size:15px;line-height:1.6;">Sua conta na <strong>${APP_NAME}</strong> foi criada com sucesso para a empresa <strong>${empresa}</strong>. Você tem <strong>${TRIAL_DAYS} dias gratuitos</strong>; o cartão é cadastrado pela Stripe e a primeira cobrança ocorre somente após o período gratuito.</p>

    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
      <tr><td style="background:#f0f9ff;border-radius:8px;padding:20px 24px;border-left:4px solid #2563eb;">
...
function buildPasswordResetHtml(name, token, expiresMinutes) {
  ...
  const body = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;"><tr><td style="background:#fef9c3;border-radius:8px;padding:16px 20px;border-left:4px solid #eab308;">
