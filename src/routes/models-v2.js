...
router.get('/models/:id/images/:variant/data-url', requireModelRead, requireCompany, async (req, res, next) => {
  try {
    ...
    const contentType = normalizeImageContentType(object.contentType || meta.content_type)
    if (!IMAGE_TYPES.has(contentType)) {
      return res.status(415).json({ error: 'unsupported_image_type', message: 'Formato de imagem não suportado.' })
    }
    ...
    return res.json({
      ...
      variant,
      data_url: `data:${contentType};base64,${object.body.toString('base64')}`
    })
  } catch (err) {
