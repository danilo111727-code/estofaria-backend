...
const auditV2Db = require('../lib/audit-v2-db')
...
function companyIdOfWebhook(record){
  ...
  const object = payload.data && payload.data.object && typeof payload.data.object === 'object'
    ? payload.data.object
    : {}
  return text(
    record?.company_id ||
...
function removeCompanyScopedCollections(store, companyId, counts){
  ...
  for(const [key, value] of Object.entries(store || {})){
    ...
    store[key] = value.filter(item => {
      if(!item || typeof item !== 'object') return true
      return text(item.company_id) !== cid
    })
...
function cleanLegacyStore(rawStore, companyId, company, actor){
  ...
  const linkedUserIds = new Set(
    store.companyUsers
      .filter(link => text(link.company_id) === cid)
      .map(link => text(link.user_id))
      .filter(Boolean)
  )
  for(const user of store.users){
    if(text(user?.company_id) === cid && user?.id) linkedUserIds.add(text(user.id))
  }
  ...
  for(const link of store.companyUsers){
    const uid = text(link?.user_id)
    if(uid && !remainingMemberships.has(uid)) remainingMemberships.set(uid, link)
  }
  ...
  store.users = store.users.filter(user => {
    ...
    if(text(user.company_id) === cid){
      user.company_id = remaining.company_id
      user.is_owner = Boolean(remaining.is_owner)
    }
    return true
  })
  ...
  store.auditLogs.unshift({
    ...
    message: `Empresa "${text(company?.name || company?.trade_name || cid)}" excluída com limpeza completa.`,
    actor_user_id: text(actor?.id),
    actor_name: text(actor?.name || actor?.email || 'Master'),
    actor_email: text(actor?.email),
    actor_role: text(actor?.role || 'master'),
    source: 'master-delete-company-v2'
  })
...
function assertLegacyClean(store, companyId){
  ...
  for(const [key, value] of Object.entries(store || {})){
    ...
    if(value.some(item => item && typeof item === 'object' && text(item.company_id) === cid)){
      throw new Error(`company_delete_verify_${key}`)
    }
  }
  if((store.webhookEvents || []).some(item => companyIdOfWebhook(item) === cid)){
    throw new Error('company_delete_verify_webhooks')
  }
}
...
async function cancelStripeSubscription(company){
  ...
  if(!stripe){
    const err = new Error('A empresa possui assinatura Stripe vinculada, mas a Stripe não está configurada no servidor.')
  ...
  try{
    ...
    return { attempted:true, status:text(canceled?.status || 'canceled') || 'canceled' }
  }catch(error){
    if(error && error.code === 'resource_missing'){
      return { attempted:true, status:'already_absent' }
    }
    const err = new Error('Não foi possível cancelar a assinatura Stripe. A empresa não foi excluída.')
...
async function verifyV2Clean(client, companyId){
  ...
  for(const table of tables){
    ...
    if(Number(result.rows[0]?.total || 0) !== 0){
      throw new Error(`company_delete_verify_${table}`)
    }
  }
}
...
async function deletePgData(companyId, company, actor){
  ...
  try{
    ...
    const auditLogs = auditV2Db.isEnabled()
      ? await client.query('DELETE FROM app_audit_logs_v2 WHERE company_id = $1', [companyId])
      : { rowCount:0 }
  ...
  // Sincroniza o cache em memória com o valor já confirmado na transação.
...
async function recordR2CleanupPending(companyId, keys){
  ...
  store.deletionCleanupJobs.push({
    id: crypto.randomUUID(),
    company_id: text(companyId),
    type: 'r2_delete',
    object_keys: keys,
    error_count: keys.length,
    status: 'pending',
    created_at: new Date().toISOString()
  })
  ...
  if(!r2.isConfigured()){
    ...
    for(const key of imageKeys){
      try{
        ...
        console.warn('[company-delete] falha ao remover R2:', key, error?.message || error)
        failed.push(key)
      }
    }
  }
...
async function deleteCompanyCompletely(companyId, actor){
  const cid = text(companyId)
  const snapshot = storeLib.readStore()
  ...
  const dbResult = storeLib._pg?.pool
    ? await deletePgData(cid, company, actor)
    : await deleteFileStoreData(cid, company, actor)
  const r2Result = await deleteR2Objects(cid, dbResult.imageKeys)

  return {
    company_name: text(company.name || company.trade_name || cid),
    stripe: stripeResult,
    legacy_deleted: dbResult.legacyCounts,
    v2_deleted: dbResult.v2Counts,
    r2: r2Result
  }
}
...
router.delete(
  ...
  requireAuth,
  requireMaster,
  requirePermission('saas.companies.write'),
  async (req, res) => {
    try{
      const result = await deleteCompanyCompletely(req.params.companyId, req.user)
      return res.json({
        ok:true,
        message:`Empresa "${result.company_name}" excluída com limpeza completa.`,
        cleanup:result
      })
    }catch(error){
      ...
      return res.status(status).json({
        error:error?.code || 'company_delete_failed',
        message:error?.message || 'Não foi possível excluir a empresa com segurança.'
      })
    }
  }
)
