(function () {
  ...
  window.initPullToRefresh = function (onRefresh) {
    ...
    // ── Eventos de toque diretos no documento shell (desktop / fallback) ──
    function scrollTop() {
      return Math.max(
    ...
    document.addEventListener('touchend', function () {
      ...
      pulling = false
      if (refreshing) return
      if (pullY < THRESHOLD) { hide(); pullY = 0; return }
      pullY = 0
      doRefresh()
    }, { passive: true })

    // ── Relay via postMessage vindo dos iframes filhos ──
    // Cada iframe injeta um script (pelo app-shell) que reenvia os toques aqui.
    window.addEventListener('message', function (e) {
      if (!e.data || typeof e.data.type !== 'string') return
      var t = e.data.type

      if (t === 'ptr-touch-start') {
        if (refreshing) return
        startY  = Number(e.data.y) || 0
        pullY   = 0
        pulling = true

      } else if (t === 'ptr-touch-move') {
        if (!pulling || refreshing) return
        var dy = Number(e.data.dy) || 0
        if (dy <= 0) { setIndicator(0); return }
        pullY = dy
        setIndicator(dy)

      } else if (t === 'ptr-touch-end') {
        if (!pulling) return
        pulling = false
        if (refreshing) return
        if (pullY < THRESHOLD) { hide(); pullY = 0; return }
